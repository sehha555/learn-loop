...
/// 講法只有兩種：給提示讓他自己算，或直接給做法（每步一行算式＋解說）。兩種都分步驟。
...
enum TeachingStyle: String, CaseIterable {
	...
	var modeRule: String {
		...
		case .direct:
			"""
			每一步就是一行算式加解說：先把這一步實際的算式寫出來（用 $ 包，例如
			「$\\int_4^5 \\frac{u-4}{2\\sqrt{u}}du$」），接著同一行用一兩句講這行是怎麼來的、
			為什麼這樣算。算式要真的算到這一步的結果，不是只描述「做代換」「移項」。不要反問他。
			"""
		}
	}
